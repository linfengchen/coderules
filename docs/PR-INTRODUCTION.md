...
```bash
# 方式 A — 项目级（推荐，可分层触发，token 最优）
curl -fsSL https://raw.githubusercontent.com/linfengchen/coderules/main/install.sh | bash -s cursor

# 方式 B — Cursor 全局（一次性覆盖本机所有项目，自动复制到剪贴板，粘贴到 Cursor 设置即可）
curl -fsSL https://raw.githubusercontent.com/linfengchen/coderules/main/install.sh | bash -s global

# 方式 C — Claude Code skill（全局 aicoding skill）
curl -fsSL https://raw.githubusercontent.com/linfengchen/coderules/main/install.sh | bash -s claude
```

**项目级 vs 全局 怎么选**：

| 维度 | 项目级 (`cursor`) | 全局 (`global`) |
|---|---|---|
| 触发方式 | always-on / glob / desc 三档 | 全部 always-on（Cursor 用户规则只是文本块） |
| Token 消耗 | ~7K 常驻 + 按需触发 | ~8K 全部常驻 |
| 多机同步 | 跟着 git 走 | 每台机器手动粘一次 |
| 项目隔离 | 是 | 否（项目级规则会覆盖全局） |
| 适合谁 | 严肃项目、团队共用 | 个人快速试用、新项目 prototype |

**推荐**：先 `global` 体验一周，确定要落地后再换成项目级（`cursor`）+ binding。

```bash
# 第一次跑完之后：
# - Cursor 中打开任意项目，问 AI 一个 200 行以上的功能
# - AI 会主动问决策维度、按 500 行限制拆文件、按四态完整画 UI

# 项目特异 binding（仅项目级模式，可选）
